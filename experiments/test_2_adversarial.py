#!/usr/bin/env python3
"""
Test 2: Adversarial attacks (SLOW on CPU - run overnight)

Run with: nohup python test_2_adversarial.py > adversarial.log 2>&1 &
"""
...
def main():
    ...
    for model_name, model in [("vanilla", vanilla), ("robust", robust)]:
        ...
        results[f"{model_name}_adversarial"] = {
            "score": pgd_report.score,
            "threshold": pgd_report.threshold
        }
        print(f"  {model_name}: {pgd_report.score:.1f}%")

        # Save individual report
        pgd_report.save(str(RESULTS_DIR / f"report_{model_name}_adversarial.json"))

    # Save combined results
    with open(RESULTS_DIR / "results_adversarial.json", "w") as f:
        json.dump(results, f, indent=2)

    print(f"\nResults saved to {RESULTS_DIR}/")
    print("Run test_3_compositional.py next")
